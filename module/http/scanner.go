...
func (scan *scan) getTLSDialer(t *zgrab2.ScanTarget) func(network, addr string) (net.Conn, error) {
	return func(network, addr string) (net.Conn, error) {
		...
		// Wrap the configuration witht the appropriate utility code
		config := (*zgrab2.TLSFlags)(&scan.scanner.config.TLSFlags)

		cfg, err := config.GetTLSConfigForTarget(t)
		if err != nil {
			return nil, err
		}
		...
		tlsConn := config.GetWrappedConnection(outer, cfg)
...
func (scanner *Scanner) newHTTPScan(t *zgrab2.ScanTarget, useHTTPS bool) *scan {
	...
	if t.Port != nil {
		...
		port = uint16(scanner.config.Base.Port)
	}
